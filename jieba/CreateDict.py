...
if __name__ == '__main__':
    ...
    # 将要转换的词库添加在这里就可以了
    o = ['E:\\workspace\\bg\\1.scel',
        'E:\\workspace\\bg\\2.scel',
         'E:\\workspace\\bg\\4.scel']

    for f in o:
        deal(f)
    import random
        # 保存结果
    f = open('uerdict.txt', 'w')
    for count, py, word in GTable:
        # GTable保存着结果，是一个列表，每个元素是一个元组(词频,拼音,中文词组)，有需要的话可以保存成自己需要个格式
        # 我没排序，所以结果是按照上面输入文件的顺序
        f.write(unicode(word + ' ' + '%d' % int(random.uniform(1,100))).encode('utf-8'))  # 最终保存文件的编码，可以自给改
        f.write('\n')
    f.close()